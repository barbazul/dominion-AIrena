import BasicAI, { CHOICE_MULTIPLY } from '../../agents/basicAI';
...
test('playEffect calls for a play choice with the actions in hand', () => {
  ...
  expect(state.current.agent.choose).toHaveBeenCalledWith(CHOICE_MULTIPLY, state, expect.arrayContaining([action1, action2, null]));
});
