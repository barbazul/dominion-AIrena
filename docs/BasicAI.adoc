...
=== BasicAi.topdeckPriority(State state, Card card, Player my): (String|Card)[]

First, try to topdeck an action that won't be played this turn.
If not possible, try topdecking the most earning treasure that we can afford
without hampering out buying power this turn.
Finally, treat the topdecking as if it were a discard and get rid of the most useless card.
